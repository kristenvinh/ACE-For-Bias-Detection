...
"""Updates made for upgrade from tf to tf2
Lines that have been updated for tf to tf2 are noted with a comment
"""
...
class ConceptDiscovery(object):
  ...
  def load_concept_imgs(self, concept, max_imgs=1000):
    ...
    img_paths = [
        ...
        ##update to tf.io.gfile.listdir(concept_dir)
  ...
  def _random_concept_activations(self, bottleneck, random_concept):
    ...
    #update to tf.io.gfile.exists for tf2
    if not tf.io.gfile.exists(rnd_acts_path):
      ...
      #updated to tf.io.gfile.GFile for tf2
  ...
  def load_cav_direction(self, c, r, bn, directory=None):
    ...
    if directory is None:
      directory = self.cav_dir
    ## Use a dictionary instead of tf.contrib.training.HParams
    ## modified from original ACE code to support TensorFlow 2.0  
    params = {'model_type': 'linear', 'alpha': 0.01}  
